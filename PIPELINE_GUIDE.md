...
Después de ejecutar la consulta, se muestra:

Algorithm used: ALGORITHM_NAME
  -> Descripción del algoritmo y columnas utilizadas

Esto te permite ver:
- Qué algoritmo se utilizó finalmente
- En qué columna se aplicó
- Parámetros de la búsqueda (valor, dirección, etc.)

Ejemplo:
Algorithm used: BINARY_SEARCH
  -> Búsqueda binaria en 'total' (DESCENDENTE) con condición 'total > 1000'
...
1. Consulta simple (FULL_SCAN):
   LEER 'data/productos.json' EXTRAER *

2. Con filtro simple (HASH_LOOKUP automático):
   LEER 'data/productos.json' EXTRAER * DONDE precio = 100

3. Con filtro y ordenamiento (BINARY_SEARCH automático):
   LEER 'data/productos.json' EXTRAER * DONDE precio > 100 ORDENAR POR precio

4. Con filtro complejo (FULL_SCAN):
   LEER 'data/productos.json' EXTRAER * DONDE precio > 100 Y stock > 20

5. Forzar algoritmo:
   LEER 'data/productos.json' EXTRAER * DONDE precio = 100 --algorithm INDEX_SCAN

6. Con todos los detalles:
   LEER 'data/ventas.json' EXTRAER id, total, cliente DONDE total > 1000 ORDENAR POR total DESC HASTA 10 --verbose

Observa cómo cambia el plan, el algoritmo y el OPTIMIZED
según tus filtros, ordenamientos y opciones.
